...
RESULT_PATH = os.path.join(OUTPUT_DIR, "incremental_cast_reuse_probe_result.json")
EXPORT_BEFORE_PATH = os.path.join(OUTPUT_DIR, "incremental_cast_reuse_probe_before_update.bplisp")
EXPORT_AFTER_PATH = os.path.join(OUTPUT_DIR, "incremental_cast_reuse_probe_after_update.bplisp")

SOURCE_ASSET = "/Game/AdvancedLocomotionV4/CharacterAssets/MannequinSkeleton/ALS_AnimBP"
TEST_ASSET = "/Game/AnimBP2FP/Phase5/ALS_AnimBP_CastReuseProbe"
GRAPH_NAME = "EventGraph"
SUCCESS_V1 = "Incremental Cast Success V1"
SUCCESS_V2 = "Incremental Cast Success V2"
...
try:
    ...
    cast_form_before = extract_form_block(export_before_dsl, "(cast")
    cast_id_before = extract_last_id(cast_form_before)
    report["export_before_success"] = get_bool(export_before, "success", "b_success")
    report["cast_id_before"] = cast_id_before
    if not report["export_before_success"] or not cast_id_before:
        raise RuntimeError("首次导出失败或未找到 cast 的 :id")

    updated_dsl = export_before_dsl.replace(SUCCESS_V1, SUCCESS_V2)
    ...
    cast_form_after = extract_form_block(export_after_dsl, "(cast")
    cast_id_after = extract_last_id(cast_form_after)
    report["export_after_success"] = get_bool(export_after, "success", "b_success")
    report["cast_id_after"] = cast_id_after
    report["contains_success_v2"] = SUCCESS_V2 in cast_form_after
    report["contains_success_v1"] = SUCCESS_V1 in cast_form_after
    report["cast_reused"] = bool(cast_id_before) and cast_id_before == cast_id_after

    if not report["export_after_success"]:
        raise RuntimeError("update 后导出失败")
    if not report["cast_reused"]:
        raise RuntimeError("cast 的 :id 在 update 前后未保持一致")
    if not report["contains_success_v2"] or report["contains_success_v1"]:
        raise RuntimeError("update 后 cast 内容未按预期更新")
