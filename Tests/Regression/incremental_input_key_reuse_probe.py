...
RESULT_PATH = os.path.join(OUTPUT_DIR, "incremental_input_key_reuse_probe_result.json")
EXPORT_BEFORE_PATH = os.path.join(OUTPUT_DIR, "incremental_input_key_reuse_probe_before_update.bplisp")
EXPORT_AFTER_PATH = os.path.join(OUTPUT_DIR, "incremental_input_key_reuse_probe_after_update.bplisp")

TEST_DIR = "/Game/AnimBP2FP/Phase5"
TEST_ASSET = f"{TEST_DIR}/BP_InputKeyReuseProbe"
GRAPH_NAME = "EventGraph"
KEY_NAME = "SpaceBar"
TEXT_V1 = "Incremental InputKey Reuse V1"
TEXT_V2 = "Incremental InputKey Reuse V2"
...
try:
    ...
    if not report["export_before_success"] or not event_id_before:
        raise RuntimeError("首次导出失败或未找到 input-key 的 :event-id")
    ...
    report["input_key_reused"] = bool(event_id_before) and event_id_before == event_id_after

    if not report["export_after_success"]:
        raise RuntimeError("update 后导出失败")
    if not report["input_key_reused"]:
        raise RuntimeError("input-key 的 :event-id 在 update 前后未保持一致")
    if not report["contains_v2"] or report["contains_v1"]:
        raise RuntimeError("update 后 input-key probe 内容未按预期更新")
