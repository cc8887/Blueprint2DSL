...
RESULT_PATH = os.path.join(OUTPUT_DIR, "incremental_call_parent_reuse_probe_result.json")
EXPORT_BEFORE_PATH = os.path.join(OUTPUT_DIR, "incremental_call_parent_reuse_probe_before_update.bplisp")
EXPORT_AFTER_PATH = os.path.join(OUTPUT_DIR, "incremental_call_parent_reuse_probe_after_update.bplisp")

SOURCE_ASSET = "/Game/AdvancedLocomotionV4/CharacterAssets/MannequinSkeleton/ALS_AnimBP"
TEST_ASSET = "/Game/AnimBP2FP/Phase5/ALS_AnimBP_CallParentReuseProbe"
GRAPH_NAME = "EventGraph"
TEXT_V1 = "Incremental CallParent Reuse V1"
TEXT_V2 = "Incremental CallParent Reuse V2"

DSL_V1 = rf'''(event
  BlueprintInitializeAnimation
  (call-parent BlueprintInitializeAnimation)
  (PrintString
    :instring "{TEXT_V1}"
    :bprinttoscreen true
    :bprinttolog true
...
def extract_call_parent_block(dsl_text):
    start = dsl_text.find("(call-parent")
    if start < 0:
        return ""
    depth = 0
...
try:
    ...
    call_parent_form_before = extract_call_parent_block(export_before_dsl)
    call_parent_id_before = extract_last_id(call_parent_form_before)
    report["export_before_success"] = get_bool(export_before, "success", "b_success")
    report["call_parent_id_before"] = call_parent_id_before
    if not report["export_before_success"] or not call_parent_id_before:
        raise RuntimeError("首次导出失败或未找到 call-parent 的 :id")
    ...
    call_parent_form_after = extract_call_parent_block(export_after_dsl)
    call_parent_id_after = extract_last_id(call_parent_form_after)
    report["export_after_success"] = get_bool(export_after, "success", "b_success")
    report["call_parent_id_after"] = call_parent_id_after
    report["contains_v2"] = TEXT_V2 in export_after_dsl
    report["contains_v1"] = TEXT_V1 in export_after_dsl
    report["call_parent_reused"] = bool(call_parent_id_before) and call_parent_id_before == call_parent_id_after

    if not report["export_after_success"]:
        raise RuntimeError("update 后导出失败")
    if not report["call_parent_reused"]:
        raise RuntimeError("call-parent 的 :id 在 update 前后未保持一致")
    if not report["contains_v2"] or report["contains_v1"]:
        raise RuntimeError("update 后 call-parent probe 内容未按预期更新")
