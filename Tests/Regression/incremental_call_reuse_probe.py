...
RESULT_PATH = os.path.join(OUTPUT_DIR, "incremental_call_reuse_probe_result.json")
EXPORT_BEFORE_PATH = os.path.join(OUTPUT_DIR, "incremental_call_reuse_probe_before_update.bplisp")
EXPORT_AFTER_PATH = os.path.join(OUTPUT_DIR, "incremental_call_reuse_probe_after_update.bplisp")

SOURCE_ASSET = "/Game/AdvancedLocomotionV4/CharacterAssets/MannequinSkeleton/ALS_AnimBP"
TEST_ASSET = "/Game/AnimBP2FP/Phase5/ALS_AnimBP_CallReuseProbe"
GRAPH_NAME = "EventGraph"
TEXT_V1 = "Incremental Call Reuse V1"
TEXT_V2 = "Incremental Call Reuse V2"
...
def extract_printstring_id(dsl_text):
    match = re.search(r'\(PrintString[\s\S]*?:id\s+"([^"]+)"\)', dsl_text or "", re.S)
...
try:
    ...
    call_id_before = extract_printstring_id(export_before_dsl)
    report["export_before_success"] = get_bool(export_before, "success", "b_success")
    report["call_id_before"] = call_id_before
    if not report["export_before_success"] or not call_id_before:
        raise RuntimeError("首次导出失败或未找到 PrintString 的 :id")
    ...
    call_id_after = extract_printstring_id(export_after_dsl)
    report["export_after_success"] = get_bool(export_after, "success", "b_success")
    report["call_id_after"] = call_id_after
    report["contains_v2"] = TEXT_V2 in export_after_dsl
    report["contains_v1"] = TEXT_V1 in export_after_dsl
    report["call_reused"] = bool(call_id_before) and call_id_before == call_id_after

    if not report["export_after_success"]:
        raise RuntimeError("update 后导出失败")
    if not report["call_reused"]:
        raise RuntimeError("PrintString 的 :id 在 update 前后未保持一致")
    if not report["contains_v2"] or report["contains_v1"]:
        raise RuntimeError("update 后 call 内容未按预期更新")
