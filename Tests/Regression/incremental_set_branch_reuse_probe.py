...
RESULT_PATH = os.path.join(OUTPUT_DIR, "incremental_set_branch_reuse_probe_result.json")
EXPORT_BEFORE_PATH = os.path.join(OUTPUT_DIR, "incremental_set_branch_reuse_probe_before_update.bplisp")
EXPORT_AFTER_PATH = os.path.join(OUTPUT_DIR, "incremental_set_branch_reuse_probe_after_update.bplisp")

SOURCE_ASSET = "/Game/AdvancedLocomotionV4/CharacterAssets/MannequinSkeleton/ALS_AnimBP"
TEST_ASSET = "/Game/AnimBP2FP/Phase5/ALS_AnimBP_SetBranchReuseProbe"
GRAPH_NAME = "EventGraph"
BRANCH_TRUE_V1 = "Incremental Branch True V1"
BRANCH_TRUE_V2 = "Incremental Branch True V2"
BRANCH_FALSE_V1 = "Incremental Branch False V1"
BRANCH_FALSE_V2 = "Incremental Branch False V2"
...
try:
    ...
    set_form_before = extract_form_block(export_before_dsl, "(set\n    JumpPlayRate")
    branch_form_before = extract_form_block(export_before_dsl, "(branch")
    set_id_before = extract_last_id(set_form_before)
    branch_id_before = extract_last_id(branch_form_before)
    report["export_before_success"] = get_bool(export_before, "success", "b_success")
    report["set_id_before"] = set_id_before
    report["branch_id_before"] = branch_id_before
    if not report["export_before_success"] or not set_id_before or not branch_id_before:
        raise RuntimeError("首次导出失败或未找到 set/branch 的 :id")

    updated_dsl = export_before_dsl.replace("1.25", "1.75", 1)
    updated_dsl = updated_dsl.replace(BRANCH_TRUE_V1, BRANCH_TRUE_V2)
    updated_dsl = updated_dsl.replace(BRANCH_FALSE_V1, BRANCH_FALSE_V2)
    ...
    set_form_after = extract_form_block(export_after_dsl, "(set\n    JumpPlayRate")
    branch_form_after = extract_form_block(export_after_dsl, "(branch")
    set_id_after = extract_last_id(set_form_after)
    branch_id_after = extract_last_id(branch_form_after)
    report["export_after_success"] = get_bool(export_after, "success", "b_success")
    report["set_id_after"] = set_id_after
    report["branch_id_after"] = branch_id_after
    report["contains_updated_set_value"] = "1.75" in set_form_after
    report["contains_true_v2"] = BRANCH_TRUE_V2 in branch_form_after
    report["contains_false_v2"] = BRANCH_FALSE_V2 in branch_form_after
    report["set_reused"] = bool(set_id_before) and set_id_before == set_id_after
    ...
    if not report["set_reused"]:
        raise RuntimeError("set 的 :id 在 update 前后未保持一致")
    if not report["branch_reused"]:
        raise RuntimeError("branch 的 :id 在 update 前后未保持一致")
    if not report["contains_updated_set_value"] or not report["contains_true_v2"] or not report["contains_false_v2"]:
        raise RuntimeError("update 后 set/branch 内容未按预期更新")
